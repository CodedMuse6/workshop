...
export const generateCertificate = functions.firestore
  .document("feedback/{feedbackId}") // submissions/{id}
  .onCreate(async (snap) => {
    const feedbackData = snap.data();

    // Load template
    // const templateBytes = fs.readFileSync(path.join
    // (__dirname, "template.pdf")
    // );

    // const pdfDocs = await PDFDocument.load(templateBytes);
    // const pages = pdfDocs.getPages()[0];
    ...
    page.drawText("Certificate of Completion", {x: 50, y: height - 100,
      size: 30, font, color: rgb(0, 0, 0.8),
    });
